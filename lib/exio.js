...
var Exio = function () {
	...
	var passToConnect = function (socket, cid, route, data, error, callback) {
		...
		if(route == '/exio_connect') {
			req.headers['X-Socket-Connect']	= 'Ex.io';
		}
		...
		return {
			...
			listen: function (server) {
				...
				io.sockets.on('connection', function (socket) {
					...
					passToConnect(socket, null, '/exio_connect', null, null, function (req) {

						if(!settings.limit || settings.limit <= this.getNumberOfClients()) {

							if(settings.authenticate.call(_this, socket, req)) {

								var clientID = settings.generate.call(this, socket, req);
								...
								settings.connect.call(_this, null, socket, req);

								for(var route in routes) {

									(function (r) {
										socket.on(r, function (data) {
											passToConnect(socket, clientID, routes[r].route, data, routes[r].error);
										});
									})(route);
								}

								socket.emit('exio:connected');

								socket.on('disconnect', function(message) {

									settings.disconnect.call(_this, null, req, message);
				...
				return {
					...
					triggerRoute: function (route, cid, data, callback) {
						...
						getSocket(cid, function (err, socket) {
							...
							passToConnect(socket, cid, routes[route].route, data, route[route].error);
						});
					},
	...
	return {
		...
		middleware: function (req, res, next) {
			...
			if(req.headers['X-Socket-Connect'] === 'Ex.io') {
